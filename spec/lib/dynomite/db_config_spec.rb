...
describe Dynomite::DbConfig do
  describe "check_dynamodb_local!" do
    it "http://localhost:8000 and port_open timeouts raises error" do
      endpoint = 'http://localhost:8000'

      allow(Dynomite::Item).to receive(:port_open?).and_return(false)
      expect { Dynomite::Item.check_dynamodb_local!(endpoint) }.to raise_error(StandardError)
    end

    it "http://localhost:8000 and port_open success " do
      endpoint = 'http://localhost:8000'
      allow(Dynomite::Item).to receive(:port_open?).and_return(true)
      expect { Dynomite::Item.check_dynamodb_local!(endpoint) }.not_to raise_error(StandardError)
    end

    it "http://dynamodb-local:8000 and port_open timeouts raises error" do
    ...
    it "http://dynamodb-local:8000 and port_open success " do
      endpoint = 'http://dynamodb-local:8000'
      allow(Dynomite::Item).to receive(:port_open?).and_return(true)
      expect { Dynomite::Item.check_dynamodb_local!(endpoint) }.not_to raise_error(StandardError)
    end
